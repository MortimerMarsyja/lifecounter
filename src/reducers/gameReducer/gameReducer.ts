      ...
      return {
        ...
        players: state.players.map((player: iPlayer) => {
          if (player.id === action.payload.id) {
            return {
              ...player,
              isAscended: action.payload.isAscended
                ? action.payload.isAscended
                : !player.isAscended,
            };
          }
          return player;
      ...
      return {
        ...
        players: state.players.map((player: iPlayer) => {
          ...
          if (player.id === action.payload.id) {
            return {
              ...player,
              hasInitiative: payload.hasInitiative
                ? payload.hasInitiative
      ...
      return {
        ...
        players: state.players.map((player: iPlayer) => {
          if (player.id === action.payload.id) {
            return {
              ...player,
              isDead: payload.isDead ? payload.isDead : !player.isDead,
            };
          }
          return player;
        }),
          ...
          if (player.id === action.payload.id) {
            return {
              ...
              commanderDamage: player.commanderDamage?.map((damage) => {
                if (damage.damage + action.payload.damage > 21) return;
                if (damage.playerId === action.payload.targetId) {
                  return {
                    ...damage,
                    damage: damage.damage + action.payload.damage,
                  };
                }
                return damage;
              }),
